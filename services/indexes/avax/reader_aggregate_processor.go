...
func (r *Reader) processorTxAscFetch(conns *utils.Connections) {
	...
	runTx := func() {
		...
		err := dressTransactions(ctx, sess, txsAsc, r.sc.GenesisContainer.DjtxAssetID, nil, false)
		if err != nil {
...
func (r *Reader) fetchAssets(
	...
	_, err := sess.Select(
		...
		Where("created_at > ? and asset_id <> ?",
			...
			r.sc.GenesisContainer.DjtxAssetID.String(),
		).
		GroupBy("asset_id").
		OrderDesc("tamt").
	...
	assets := append([]string{}, r.sc.GenesisContainer.DjtxAssetID.String())
