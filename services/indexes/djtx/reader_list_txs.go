...
func (r *Reader) ListTransactions(ctx context.Context, p *params.ListTransactionsParams, djtxAssetID ids.ID) (*models.TransactionList, error) {
	...
	if !dressed {
		if err := dressTransactions(ctx, dbRunner, txs, djtxAssetID, p.ListParams.ID, p.DisableGenesis); err != nil {
			return nil, err
		}
	}
...
func dressTransactions(
	...
	dressTransactionsTx(txs, disableGenesis, txID, djtxAssetID, inputsMap, outputsMap, inputTotalsMap, outputTotalsMap, rewardsTypesMap, cvmin, cvmout)
	return nil
}
...
func dressTransactionsTx(
	...
	for _, tx := range txs {
		if disableGenesis && (txID == nil && string(tx.ID) == djtxAssetID.String()) {
			continue
		}
