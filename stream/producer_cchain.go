...
import (
	...
	caminoGoUtils "github.com/lasthyphen/dijetsnodego/utils"
...
type producerCChainContainer struct {
	...
	catchupErrs caminoGoUtils.AtomicInterface
}
...
func newContainerC(
	...
	cl, err := modelsc.NewClient(conf.CaminoGO + "/ext/bc/U/rpc")
	if err != nil {
		_ = conns.Close()
		return nil, err
	}
...
func (p *ProducerCChain) runProcessor() error {
	...
	for icnt := 0; icnt < maxWorkers; icnt++ {
		cl, err := modelsc.NewClient(p.conf.CaminoGO + "/ext/bc/U/rpc")
		if err != nil {
			return err
		}
		conns1, err := p.sc.Database()
